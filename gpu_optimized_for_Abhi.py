...
def run_transicp_parallel(blocks):
    ...
    return np.stack([X, Y, dx, dy, dz], axis=1)
...
# Access shared memory in worker processes
def transicp_worker(shared_name, shape, dtype, config, x, y):
    existing_shm = shared_memory.SharedMemory(name=shared_name)
    array = np.ndarray(shape, dtype=dtype, buffer=existing_shm.buf)
    # Process the array
    result = transicp(array, array, array, config, x, y)
    existing_shm.close()
    return result


def set_cpu_affinity():
    """Set CPU affinity to the first 16 cores."""
    p = psutil.Process()
    p.cpu_affinity([i for i in range(16)])  # Use cores 0 to 15
''' ---------------------------------------------------------------------- '''


if __name__ == "__main__":

    # Set CPU affinity at the start of the main process
    set_cpu_affinity()
    
    ''' ------------ Set the following parameters ----------------- '''
    # pre_event = r'D:\Working\SCE\Landslide\Data\Overlap-Analysis\Line1310-Ground-Overlap.las'
    # pos_event = r'D:\Working\SCE\Landslide\Data\Overlap-Analysis\Line1311-Ground-Overlap.las'
    ...
    output_names = ['klondike_0925_1018_']
    window_size = 150
    step_size = 50
    ...
    for i in range(len(events) - 1): 
        ...
        if operation == 'translation_only':
            ...
            'null': -99,
            'output_basename' : output_names[i] #'trans_icp_results_0906_0925¥'
            }
        
        config = icp_configs(configs)
    
        a = run_all(events[i], events[i+1], config)
